...
function renderDashboard() {
  ...
  el('overdue-list').innerHTML = overdue.length
    ? sort(overdue).map(t=>taskCard(t)).join('') : '<div class="empty">No overdue tasks 🎉</div>';
  el('due-soon-list').innerHTML = soon.length
    ? sort(soon).map(t=>taskCard(t)).join('') : '<div class="empty">Nothing due this week</div>';
  el('upcoming-list').innerHTML = upcoming.length
    ? sort(upcoming).map(t=>taskCard(t)).join('') : '<div class="empty">Nothing in the next 60 days</div>';
}
...
function renderModuleDetail(code) {
  ...
  // Learning units — each unit gets a tickable checkbox with a unique id
  ...
  const unitsHtml = units.length ? `
    ...
    ` + units.map((u,i) => {
      ...
      const done = doneTasks.includes(uid);
      return `<div class="unit-card ${done?'unit-done':''}">
        <div class="unit-check-wrap">
          <div class="checkbox ${done?'checked':''}" onclick="toggleDone('${uid}')" title="${done?'Mark as not done':'Mark as studied'}"></div>
        </div>
  ...
  // Assessments — each with a checkbox + bulk toolbar
  ...
  const assessHtml = allA.length ? `
    ...
    ` + allA.map(a => {
      ...
      return `<div class="item-card ${done?'done':''}">
        <div class="checkbox ${done?'checked':''}" onclick="toggleDone('${id}')"></div>
        <div class="dot" style="background:${DOT_COLORS[u]};flex-shrink:0;margin-top:5px;"></div>
        <div class="item-body">
          <div class="item-title">${a.title}</div>
          <div class="item-meta">${a.type}${openedStr?' · '+openedStr:''}${dueStr?' · '+dueStr:''}</div>
          ${urgencyBadge(due, done)}
...
function renderModuleCalendar(code) {
  const y = moduleCalMonth.getFullYear(), m = moduleCalMonth.getMonth();
...
function addManual() {
  ...
  const task = { id: 'custom_'+Date.now(), title, module, type, due: due||null, note, custom: true };
...
function initSchedulePage() {
  ...
  list.innerHTML = MODULE_DATA.map(mod => {
    const c = col(mod.color || 0);
    const pendingAssess = [...(mod.assessments||[]),...(mod.tasks||[])].filter(a => !doneTasks.includes(`${mod.code}__${a.title}`)).length;
    const pendingUnits  = (mod.learningUnits||[]).filter((_,i) => !doneTasks.includes(`${mod.code}__unit__${i}`)).length;
...
function generateSchedule() {
  ...
  // ── Step 1: Build ordered item sequences per module ───────
  // For each module we produce an ordered list of "study blocks":
  //   [unit A, unit B, ASSESSMENT X, unit C, ASSESSMENT Y, ...]
  // Units always appear before the assessment they feed into.
  ...
  selectedMods.forEach(code => {
    ...
    // Track which unit indices have already been scheduled for this module
    const scheduledUnitIdxs = new Set();

    // Walk assessments in their natural order (which matches due date order)
    assessList.forEach(a => {
      ...
      // Schedule any required units that haven't been scheduled yet
      if (incUnits) {
        reqIdxs.forEach(i => {
          if (scheduledUnitIdxs.has(i)) return; // already added earlier
          const u      = (mod.learningUnits||[])[i];
          if (!u) return;
          const uId   = `${code}__unit__${i}`;
          const uDone = doneTasks.includes(uId);
          if (!incDone && uDone) { scheduledUnitIdxs.add(i); return; }
          ...
          allOrderedItems.push({
            ...
            title:       u.title,
            id:          uId,
            due:         aDue,        // soft deadline = same as its assessment
            hardDue:     aDue,
            done:        uDone,
            hours:       1.5,
            color:       mod.color || 0,
      ...
      // Now schedule the assessment itself
      if (incAssess) {
        if (!incDone && aDone) return;
        const hrs = a.type==='Assignment' ? 4 : a.type==='Quiz' ? 1
          : a.type==='Exam' ? 3 : a.type==='Test' ? 2 : 1.5;
        allOrderedItems.push({
          module:      code,
          moduleTitle: mod.title,
          type:        a.type || 'Assessment',
    ...
    // Any learning units NOT linked to any assessment — append at end
    if (incUnits) {
  ...
  // ── Step 2: Build available study days ────────────────────
  const availableDays = [];
  ...
  const schedule = availableDays.map(d => ({ date: new Date(d), items: [], hoursUsed: 0, modules: new Set() }));

  // ── Step 3: Interleave 2 modules per day ──────────────────
  // Strategy:
  //  - Split allOrderedItems into per-module queues (order preserved = dependency order)
  //  - Each day: pick items from UP TO 2 different modules, filling the hour budget
  //  - Within each module the queue order is NEVER broken (unit always before its assessment)
  //  - Hard deadlines are respected by urgency-sorting the module queues each day

  // Build per-module queues
  const moduleQueues = {}; // code → [ ...items in dependency order ]
  ...
  schedule.forEach((day, dayIdx) => {
    ...
    // Determine which modules have urgent items due soon — prioritise those
    ...
    // Try to fill from up to 2 modules per day
    for (const code of modsByUrgency) {
      if (!moduleQueues[code] || moduleQueues[code].length === 0) continue;
      if (pickedMods.size >= 2) break; // max 2 modules per day

      const queue = moduleQueues[code];
      let addedFromThisMod = false;

      // Keep pulling from this module's queue while there's budget and items
      while (queue.length > 0 && hoursLeft > 0) {
        const item = queue[0];

        // Respect hard deadline — don't schedule this item past its due date
        if (item.hardDue) {
          const dueD = new Date(item.hardDue); dueD.setHours(23,59,59);
          if (day.date > dueD) {
            // This item is already past due on this day — skip module for today,
            // it will be force-placed later
            break;
          }
        }

        // Check if item fits in remaining budget (allow slight overflow for last item)
        const fits = item.hours <= hoursLeft + 0.5;
        if (!fits && addedFromThisMod) break; // already placed something from this mod today, stop
        if (!fits && !addedFromThisMod) {
          // Nothing placed yet from this mod today — try anyway (partial day)
          if (item.hours > budget + 1) break; // item is just too big, skip for now
        }
        ...
        // Only place ONE unit or ONE assessment per module per day — keeps variety
        // Exception: if the unit and its assessment are tiny (quiz + unit = ≤2.5h total), do both
        const nextInQueue = queue[0];
        const bothSmall = nextInQueue && !nextInQueue.isUnit && item.isUnit && (item.hours + nextInQueue.hours) <= 2.5;
        if (!bothSmall) break; // stop after first item from this module (unless both are tiny)
      }
    }

    // ── Force-place any items whose deadline is TODAY (can't skip them) ──
    for (const code of modCodes) {
      ...
      const dueD = new Date(item.hardDue); dueD.setHours(23,59,59);
      if (day.date >= dueD) {
        // Must place today regardless of budget/module limit
        queue.shift();
        day.items.push(item);
        day.hoursUsed += item.hours;
      }
  ...
  // ── Anything left in queues? Append to last available days ──
  modCodes.forEach(code => {
    while (moduleQueues[code] && moduleQueues[code].length > 0) {
      const item = moduleQueues[code].shift();
      // Find last day with some room, or just append to final day
      let placed = false;
...
// We persist the generated schedule so we can detect missed days on reload.
let savedScheduleSettings = null; // { endDate, hoursDay, studyDays, selectedMods, incUnits, incAssess, incDone }

function renderScheduleOutput(days, allItems, endDate, hoursDay) {
  // Save the schedule settings so "Reschedule" can regenerate with same preferences
  savedScheduleSettings = {
    endDate, hoursDay,
    studyDays: Array.from(document.querySelectorAll('.day-picker input[type=checkbox]'))
      .filter(c => c.checked).map(c => parseInt(c.value)),
    selectedMods: Array.from(document.querySelectorAll('.sched-mod-cb'))
      .filter(c => c.checked).map(c => c.value),
    incUnits:   document.getElementById('inc-units').checked,
    incAssess:  document.getElementById('inc-assessments').checked,
    incDone:    document.getElementById('inc-done').checked,
  };
  localStorage.setItem('schedSettings', JSON.stringify(savedScheduleSettings));
  ...
  // Count missed items: past days where items are not ticked
  const missedItems = [];
  days.forEach(day => {
  ...
  // Show reschedule alert banner if there are missed items
  ...
  days.forEach(day => {
    ...
    const dayName   = day.date.toLocaleDateString('en-ZA',{weekday:'long'});
    const dayStr    = day.date.toLocaleDateString('en-ZA',{day:'numeric',month:'short'});
    const hrs       = Math.round(day.hoursUsed * 10) / 10;

    // Day status for past days
    let dayStatus = '';
    ...
    const byMod = {};
    day.items.forEach(item => {
      if (!byMod[item.module]) byMod[item.module] = { title: item.moduleTitle, color: item.color, items: [] };
    ...
    const modBlocks = Object.entries(byMod).map(([code, group]) => {
      ...
      const rows = group.items.map(item => {
        ...
        // Inline tick checkbox — clicking marks it done without leaving the schedule
...
function schedToggleDone(id) {
  ...
  localStorage.setItem('doneTasks', JSON.stringify(doneTasks));
  // Re-render the schedule output in place (without going back to form)
  rescheduleCurrentView();
  // Also refresh the rest of the app
  renderDashboard();
  if (activeModule) renderModuleDetail(activeModule);
  updateModuleFilter();
}

// Reschedule missed items — regenerates from today using saved settings
function rescheduleFromToday() {
  ...
  // Re-run generate with incDone=false so already-done items are skipped,
  // and today as the effective start date (the generator always starts from today)
  if (!savedScheduleSettings) { generateSchedule(); return; }

  // Restore form values from saved settings then regenerate
  ...
  // Re-init module list (rebuilds checkboxes) then restore selections
  initSchedulePage();
  setTimeout(() => {
    ...
    document.getElementById('inc-done').checked    = false; // never include already-done on reschedule
...
// Re-render the current schedule output without going back to form
// (used after ticking items directly in the schedule)
